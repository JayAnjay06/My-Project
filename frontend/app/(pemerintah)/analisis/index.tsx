...
export default function AnalisisDanKeputusan() {
  ...
  // FUNGSI BUAT KEPUTUSAN BARU
  const handleBuatKeputusan = () => {
    if (!hasil) {
      Alert.alert("Peringatan", "Hasil analisis belum tersedia");
      return;
    }
    // Pre-fill form dengan rekomendasi AI
    setFormData({
      tindakan_yang_diambil: hasil.rekomendasi_penanganan || '',
      anggaran: '',
      tanggal_mulai: '',
      tanggal_selesai: '',
      catatan: ''
  ...
  const submitKeputusan = async () => {
    ...
    try {
      ...
      console.log("Mengirim keputusan:", payload);

      const res = await fetch(`${API_URL}/keputusan`, {
        method: "POST",
        headers: {
      ...
      if (json.success) {
        // Refresh data keputusan
        await fetchData();
        setShowKeputusanModal(false);
        Alert.alert("Sukses", "Keputusan berhasil dibuat");
      } else {
        throw new Error(json.message || "Gagal membuat keputusan");
      }
    } catch (err: any) {
      console.error("Gagal buat keputusan:", err);
      Alert.alert("Error", err.message || "Gagal membuat keputusan");
    } finally {
      setKeputusanLoading(false);
    }
  };

  // FUNGSI HAPUS KEPUTUSAN
  const handleHapusKeputusan = async (keputusanId: number) => {
    Alert.alert(
      "Konfirmasi Hapus",
        ...
        },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              const res = await fetch(`${API_URL}/keputusan/${keputusanId}`, {
                method: "DELETE",
                headers: {
                  Authorization: `Bearer ${token}`,
                },
              ...
              if (json.success) {
                ...
                throw new Error(json.message || "Gagal menghapus keputusan");
              }
            } catch (err: any) {
              console.error("Gagal hapus keputusan:", err);
              Alert.alert("Error", err.message || "Gagal menghapus keputusan");
            } finally {
                  ...
                  <View key={keputusan.keputusan_id} style={styles.keputusanCard}>
                    ...
                    <View style={styles.cardHeader}>
                      ...
                      <TouchableOpacity 
                        ...
                        onPress={() => handleHapusKeputusan(keputusan.keputusan_id)}
      ...
      >
        <View style={styles.modalContainer}>
          <View style={styles.modalHeader}>
            ...
            <TouchableOpacity 
              onPress={() => setShowKeputusanModal(false)}
          ...
          <ScrollView style={styles.modalContent}>
            <View style={styles.inputContainer}>
              ...
              <TextInput
                ...
                onChangeText={(text) => setFormData({...formData, tindakan_yang_diambil: text})}
            ...
            <View style={styles.rowInputs}>
              <View style={[styles.inputContainer, { flex: 1 }]}>
                ...
                <TextInput
                  ...
                  onChangeText={(text) => setFormData({...formData, tanggal_mulai: text})}
              ...
              <View style={[styles.inputContainer, { flex: 1 }]}>
                ...
                <TextInput
                  ...
                  onChangeText={(text) => setFormData({...formData, tanggal_selesai: text})}
                  placeholder="YYYY-MM-DD"
                />
              </View>
            </View>
          ...
          <View style={styles.modalFooter}>
            ...
            <TouchableOpacity 
              style={[styles.submitButton, keputusanLoading && styles.buttonDisabled]}
              onPress={submitKeputusan}
              disabled={keputusanLoading}
            >
              {keputusanLoading ? (
                <ActivityIndicator color="white" size="small" />
              ) : (
